...
const Header = () => {
  ...
  const profileRef      = useRef(null);
    ...
    const fetchList = (page = 1) => {
        ...
        postRequestWithToken('plusx-notification-list', obj, async (response) => {
            if (response.code === 200) {
                setNotifications(response?.data);
                // setTotalPages(response?.total_page || 1);
                // setTotalCount(response?.totalRows || 0)  href_url
            } else {
                console.log('error in plusx-notification-list api', response);
            }
            setLoading(false);
        });
    };
    ...
    const getNotificationCount = () => {
        ...
        postRequestWithToken('plusx-notification-list', objCount, async (response) => {
            if (response.code === 200) {
    ...
    useEffect(() => {
        ...
        if( process.env.REACT_APP_SERVER_URL != 'http://localhost:2424/' ) {
            ...
            socket.on('plusx-notification-list', (data) => {
    ...
    const handleNavigate = (module, hrefUrl) => {
        ...
        switch (module) {
            case "Portable Charging Booking":
                navigate(`/electric/home-charger/charger-booking-details/${extractedId}`);
                break;

            case "Roadside Assistance":
                navigate(`/electric/ev-road-assistance/booking-details/${extractedId}`);
                break;
            ...
            case "Charging Installation Service":
                navigate(`/electric/charger-installation/charger-installation-details/${extractedId}`);
                break;
            
            case "EV Accessories Booking":
                navigate(`/electric/charger-installation/accessories-booking-details/${extractedId}`);
                break;   
            ...
            <div className={styles.notificationContainer} onClick={toggleNotification} ref={notificationRef}>
                ...
                <div className={styles.notificationDropdown}>
                    ...
                    <div className={styles.notificationBottomSection}>
                        <div className={styles.notificationCount}>{notifications.length} Notifications</div>
                        <Link to={`/${selectedApp}/notification-list`}><div className={styles.notificationAllList}>See All</div></Link>
                    </div>
                </div>
                )}
            </div>
