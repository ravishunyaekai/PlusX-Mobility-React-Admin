...
const EvChargerDetails = () => {
    ...
    const sectionContent1 = {
       ...
       used_for       : bookingDetails?.used_for?.map(u => u.label).join(", "),
       property_type  : bookingDetails?.property_type?.map(p => p.label).join(", "),
    ...
    const handleRemoveCoverImage = (galleryId) => {
        ...
        if (confirmDelete) {
            ...
            postRequestWithToken('ev-chargers-cover-del', obj, async (response) => {
                if (response.code === 200) {
                    toast(response.message, { type: "success" });
                    setTimeout(() => {
                    fetchDetails();
            ...
            { loading ? <Loader /> : 
                <>
                    ...
                    <div className={styles.bookingLeftContainer}>
                        ...
                        <BookingImageSection titles={imageTitles} content={imageContent} type='evChargerDetails' onRemoveImage={handleRemoveCoverImage} />
                        <BookingMultipleImages titles={imageTitles} content={imageContent} type='EvchargerDetailGallery' onRemoveImage={handleRemoveGalleryImage} />
