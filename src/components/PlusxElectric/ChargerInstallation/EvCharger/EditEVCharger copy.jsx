...
const EditEVCharger = () => {
    ...
    // };
          ...
          const handleRemoveGalleryImage = (index) => {
            console.log("galleryId",index)
        const confirmDelete = window.confirm(`Do you want to delete this item?${index}` );
        if (confirmDelete) {
            const obj = { 
                userId     : userDetails?.user_id,
                email      : userDetails?.email,
            ...
            postRequestWithToken('xev-charger-gallery-del', obj, async (response) => {
                if (response.code === 200) {
                    toast(response.message, { type: "success" });
