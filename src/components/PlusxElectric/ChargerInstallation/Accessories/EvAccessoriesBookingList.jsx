...
const EvAccessoriesrBookingList = () => {
    const userDetails                   = JSON.parse(sessionStorage.getItem('userDetails')); 
    const navigate                      = useNavigate();
    const [accessoriesBookingList, setAccessoriesBookingList] = useState([]);
    ...
    const fetchList = (page, appliedFilters = {}) => {
        ...
        postRequestWithToken('charger-installation-list', obj, async(response) => {
            if (response.code === 200) {
                setAccessoriesBookingList(response?.data)
                setTotalPages(response?.total_page || 1); 
                setTotalCount(response?.total||0);
            } else {
                // toast(response.message, {type:'error'})
                console.log('error in accessories-booking-list api', response);
            }
            setLoading(false);
        })
    }
    ...
    return (
        <div className='main-container'>
            <SubHeader 
                heading            = "EV Accessories Bookings"
            ...
            />
            {loading ? <Loader /> :
                accessoriesBookingList.length === 0 ? (
                    <EmptyList
                ...
                <>
                    <List 
                        ...
                        listData = {accessoriesBookingList}
...
export default EvAccessoriesrBookingList;
